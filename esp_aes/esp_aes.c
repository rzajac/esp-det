...
uint32_t esp_aes_encrypt(uint8_t *output, uint8_t *input, uint32_t length, const uint8_t *key, const uint8_t *iv)
{
  ...
  const uint8_t remainders = (const uint8_t) (length % KEYLEN); /* Remaining bytes in the last non-full block */
  uint32_t ret = (uint32_t) (full_blocks * KEYLEN);
  ...
  if(remainders)
  {
    ...
    memset(output + remainders, 0, (size_t) (KEYLEN - remainders)); /* add 0-padding */
    XorWithIv(output);
    state = (state_t*)output;
    Cipher();
    ret += KEYLEN;
...
uint32_t esp_aes_decrypt(uint8_t *output, uint8_t *input, uint32_t length, const uint8_t *key, const uint8_t *iv)
{
  ...
  {
    FragmentCopy(output, input, remainders);
    os_memset(output+remainders, 0, (unsigned int) (KEYLEN - remainders)); /* add 0-padding */
    state = (state_t*)output;
    InvCipher();
    ret += KEYLEN;
  }
